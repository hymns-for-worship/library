...
void main() {
  group('import hymn', () {
    ...
    setUp(() {
      db = DriftStorage(NativeDatabase.memory());
      importer = ImportHymn(db);
    });
    ...
    test('Valid HIXML Import', () async {
      final id = await importer.importInfo(HYMN_002.trim());

      expect(id, '000000000000004');

      final hymn = await db.docs.doc('hymns', id).get();

      expect(hymn != null, true);
      expect(hymn!.id, id);
      expect(hymn.data!['number'], '002');
      expect(hymn.data!['title'], 'Hallelujah! Praise Jehovah');

      final hymnals = await db.docs.collection('hymnals').getAll();
      final hymnal = hymnals.first;

      expect(hymnal.id, '000000000000001');
      expect(hymnal.data!['name'], 'Hymns For Worship (Revised)');
      expect(hymnal.data!['alias'], 'HFWR');
    });

    test('Invalid HIXML Import', () async {
